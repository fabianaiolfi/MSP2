...
wc_plot <- all_businesses_wc %>% 
  left_join(all_businesses_eda, by = "BusinessShortNumber") %>% 
  left_join(all_businesses_party, by = "BusinessShortNumber")
...
plot_time <- wc_all_bills %>% left_join(select(eda, BusinessShortNumber, StartDate), by = "BusinessShortNumber")

ggplot(plot_time, aes(x = as.factor(StartDate), y = SubmittedText_clean_wc)) +
  geom_boxplot() +
  labs(x = " ", y = "Word Count", title = "Word Count by Session") +
  scale_x_discrete(breaks = NULL) + # remove y-axis labels
  theme_minimal()
