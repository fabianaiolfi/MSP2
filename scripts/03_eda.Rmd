...
all_businesses_eda <- eda %>% left_join(select(all_businesses_eda, -TagNames, -SubmissionSession), by = "BusinessShortNumber")
all_businesses_party <- eda %>% left_join(select(all_businesses_party, -PartyAbbreviation), by = "BusinessShortNumber")
```
...
session_leg_index <- all_sessions %>% 
  select(ID, LegislativePeriodNumber) %>% 
  distinct(ID, .keep_all = T)

eda_environment_sp_bills_vote_share <- eda %>% 
  select(-StartDate, -year_session) %>% 
  dplyr::filter(grepl(set_tag, TagNames)) %>% 
  dplyr::filter(grepl("^SP$", PartyAbbreviation)) %>% 
