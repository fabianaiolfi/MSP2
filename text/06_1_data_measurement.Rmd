...
- Table \ref{tab:counts} provides an overview of the number of bills introduced in the Swiss parliament between March 2004 and December 2023. 
```{r, child = 'tables/counts.Rmd'}
```

- Figure \ref{fig:all-bills} shows how the number of all parliamentary bills has increased in the past 20 years. The number of bills introduced by the SP and GPS have mainly stayed constant, except for an uptick after the 2019 election, where the GPS had their best election result in their history
- When comparing the number of bills tagged with the environmental label to all bills, the number of environmental bills is mostly flat until the early 2010s, where we observe a slow increase up until the late 2010s, after which there is a strong increase (see Figure \ref{fig:all-bills})
```{r, child = 'figures/all_bills.Rmd'}
```

<!--```{r, child = 'figures/env_bills.Rmd'} -->
<!-- ``` -->

- If we break down the number of environmental bills by party, we can see that all parties introduced roughly the same number of bills up until the late 2010s, where the SP and GPS started introducing much more of these bills. Then all parties decrease again except for the GPS. this makes intuitive sense, as the green parties owns green topics and had their biggest success in the 2019 national elections, thus giving them a boost. (see Figure \ref{fig:env-party-bills})
...
Figure \ref{fig:wc-bills} displays that the number of words in all bills (median: 72 words) and the number of words in environmental bills (median: 74 words) are roughly the same. when looking at the number of words in bills over time, values are also roughly stable. The standard deviation of the median for each year is 7.72 for all bills and 12.49 for environmental bills. we can also observe a similar stability in environmental bills introduced by the sp and the gps. the standard deviation of the median over time is 13.77 for sp bills and 14.95 for green bills. in sum, this examination shows that the number of words per bill stays roughly the same over time. this is beneficial for applying a text as data approach, as there is roughly the same amount of data available over the entire time span under examination.
...
A brief visual comparison of both measures show that both Selects and Google Trends generally follow a similar trend line on environmental issues. This underlines the legitimacy of using Google Trends as a way of measuring public salience of environmental topics. See Figure \ref{fig:gt-selects}.
...
- When examining all types of bills put forward by the SP and the GPS, we can see that the average distance between all bills is roughly constant over time (see figure \ref{fig:embeddings-all-bills})
- on the other hand, when only examining environmental bills, we can see a clear downwards trend over the past twenty years. this downward line means that the distance between these bills has decreased over time, indicating that they have become semantically more similar
- please note that the distance values cannot be interpreted by themselves. e.g., a distance of value $3$ has no meaning in and of itself. these values must be interpreted as a comparison. thus, a value of $2$ is lower than a value of $3$. in this context this means that two groups of bills with a shorter distance are semantically more similar
...
This outlier, with a value of 100, causes the rest of the data series to display relatively low variation due to the way Google Trends scales values (see line A in Figure \ref{fig:gt-index}). This low variation becomes more apparent when comparing the Google Trends data up until March 2022 (see line B in Figure \ref{fig:gt-index}). Low variation is problematic for modeling the data later on, as it makes effects less pronounced. To restore the variation lost in the time series that includes April 2022, I recalculated each value by following these steps: First, I calculate the relative change of each value in the shorter series with respect to the first value, obtaining the "multiplier." Second, I align the starting points of both series by adding a value of 42 to the longer series. Third, I multiply each value in the longer series by the "multiplier" from the first step. This results in a more pronounced variation in the longer series that mirrors the shorter series (see line C in Figure \ref{fig:gt-index}).
