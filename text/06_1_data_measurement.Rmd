...
<!--
to do: explain that you’re using “SubmittedText” to analyse the content of bills, and explain what this part is of a bill
-->
...
In the next step, I examine the lengths of the bills introduced by counting the number of words in each bill. This is relevant because calculating the embedding of a bill requires calculating the embedding of each word. Consequently, the more words in a single bill, the more information is available, leading to a more precise placement of the bill in semantic space. If the number of words varied significantly between bills, this could have a negative impact on the accuracy of the analysis.
...
In summary, this brief examination shows that the number of words per bill remains fairly consistent over time. This consistency is beneficial for the application of a text-as-data approach, as it ensures that a comparable amount of data is available per bill throughout the period under study.
