...
\begin{equation*}
\begin{aligned}
\Delta \; Euclidean \; Distance_{s+1} = \beta \; + \; \Delta \; Search \; Term \; Popularity_{s} \; + \; \epsilon
\end{aligned}
\end{equation*}

In these models, Delta (Δ) indicates the change between two observations, specifically between two parliamentary sessions. The variable $s+1$ represents the lagged variable, where s denotes the current parliamentary session and $s+1$ denotes the next session.
